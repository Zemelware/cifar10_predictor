...
while True:
    if len(sys.argv) < 2:
        interface = input(
            "\nWould you like to use a web interface or terminal interface? (w/t) ")
    else:
        interface = 't'
    ...
    if interface == 'w':
        ...
        gr.Interface(fn=classify_image, inputs=gr.Image(
            shape=(32, 32)), outputs="label").launch()
        break
    elif interface == 't':
        first_time = True
        while True:
            ...
            for i, (category, probability) in enumerate(prediction):
                if i == 0:
                    print('\033[92m', end='')  # Print the prediction in green
        ...
        print(
            "\033[91mInvalid option, please enter 'w' for web interface or 't' for terminal interface.\033[0m")
